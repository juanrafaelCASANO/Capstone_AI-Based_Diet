...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; background: #f4f7ff; color: #0f172a; overflow-x: hidden; }
        ...
        .tag { background:#e0e7ff; color:#2563eb; padding:6px 14px; border-radius:12px; font-size:12px; }
        .btn { display:inline-block; margin-top:15px; background:#2563eb; color:#fff; padding:12px 22px; border-radius:14px; font-weight:600; text-decoration:none; }
        .avatar { width:120px; height:120px; border-radius:50%; object-fit:cover; border: 4px solid #e0e7ff; }
        .price { color:#2563eb; font-weight:700; }
        .filter { margin-bottom:30px; }
        select { padding:12px; border-radius:10px; border:1px solid #cbd5e1; }
        .no-results { grid-column:1/-1; text-align:center; padding:50px; background:#fff; border-radius:20px; color:#64748b; font-weight:600; }
    </style>
</head>
...
<body>
    ...
    <div class="container">
        ...
        <?php
        if ($goal) {
            $sql = "SELECT * FROM meal_plans WHERE goal = ? ORDER BY id DESC";
        ...
        ?>
            <div class="card">
                <img src="<?= htmlspecialchars($img) ?>" alt="<?= htmlspecialchars($row['title']) ?>" style="width:100%;height:180px;object-fit:cover;border-radius:16px;margin-bottom:15px;">
        ...
        <?php
        if ($specialty) {
            $sql = "SELECT * FROM nutritionist WHERE certification=? AND status='approved' ORDER BY id DESC";
            $stmt = $conn->prepare($sql);
