...
// 2. Wrap your query in a try-catch or check if column exists
try {
    $msg_query = "SELECT COUNT(*) as total FROM messages WHERE receiver_id = $nutri_id";
    // If you added the 'status' column in the previous step, use this instead:
    // $msg_query = "SELECT COUNT(*) as total FROM messages WHERE receiver_id = $nutri_id AND status = 'unread'";
    ...
    // If the query fails, $unread_count stays 0 and won't throw a warning
    $unread_count = 0;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        body{font-family:Arial; background:#f0fdf4; margin:0} /* Greenish theme for Health */
