...
if(isset($_POST['update'])){
    ...
    if (!empty($_POST['new_password'])) {
        ...
        $success = $stmt->execute([$name, $email, $exp, $new_pass, $id]);
    } else {
        $stmt = $conn->prepare("UPDATE nutritionist SET fullname=?, email=?, experience=? WHERE id=?");
        $success = $stmt->execute([$name, $email, $exp, $id]);
    }
...
<head>
    ...
    <style>
        ...
        .form-card { background: #fff; max-width: 500px; margin: auto; padding: 30px; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); }
        ...
        input { width: 100%; padding: 12px; margin-bottom: 20px; border: 1px solid #e2e8f0; border-radius: 10px; box-sizing: border-box; font-family: inherit; }
        .btn-update { background: #2563eb; color: white; border: none; padding: 14px; width: 100%; border-radius: 10px; cursor: pointer; font-weight: 700; }
        .btn-cancel { display: block; text-align: center; margin-top: 15px; color: #94a3b8; text-decoration: none; font-size: 14px; }
